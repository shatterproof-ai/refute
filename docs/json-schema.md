...
| `unsupported` | `unsafe-refactor` | `1` | The backend supports the operation in general but refused this specific invocation because it could not be performed safely (for example `change-signature` on a parameter still in use, or `move` onto a cross-package destination, an existing destination file, or a symbol the backend cannot move). Refusal happens before any edit, so the workspace is unchanged; `error.message`/`error.hint` names the constraint. Additive under `schemaVersion: "1"`. |
