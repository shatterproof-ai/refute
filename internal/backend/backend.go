...
// layers can surface a specific, actionable refusal. Refusal happens before any
// edit is applied, so it is preview-safe: a refused operation in --dry-run
// prints the refusal and changes nothing, identically to apply mode. Shared by
// both fixed-arity operations (e.g. move, via RefactoringBackend) and
// parameterized ones (e.g. change-signature, via ParameterizedBackend). See
...
type ErrUnsafeRefactor struct {
	...
	Code      string // stable, e.g. "cross-package-move", "unmovable-symbol", "parameter-in-use"
}
